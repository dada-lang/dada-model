                ...
                3: judgment `can_type_expr_as { expr: { let foo = new Foo () ; foo ; }, as_ty: Foo, env: Env { program: class Foo { } class TheClass { fn empty_method (Some(my self)) -> Foo { let foo = new Foo () ; foo ; } }, universe: universe(0), in_scope_vars: [], local_variables: [self : my TheClass], existentials: [], assumptions: {} }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                     the rule "can_type_expr_as" failed at step #0 (src/file.rs:LL:CC) because
                       judgment `type_expr_as { expr: { let foo = new Foo () ; foo ; }, as_ty: Foo, env: Env { program: class Foo { } class TheClass { fn empty_method (Some(my self)) -> Foo { let foo = new Foo () ; foo ; } }, universe: universe(0), in_scope_vars: [], local_variables: [self : my TheClass], existentials: [], assumptions: {} }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                         the rule "type_expr_as" failed at step #1 (src/file.rs:LL:CC) because
                           judgment `sub { a: shared (foo) Foo, b: Foo, env: Env { program: class Foo { } class TheClass { fn empty_method (Some(my self)) -> Foo { let foo = new Foo () ; foo ; } }, universe: universe(0), in_scope_vars: [], local_variables: [self : my TheClass, foo : Foo], existentials: [], assumptions: {} }, flow: Flow { moved_places: {self} } }` failed at the following rule(s):
                             the rule "collapse a or b" failed at step #3 (src/file.rs:LL:CC) because
                               judgment `sub { a: shared (foo) Foo, b: my Foo, env: Env { program: class Foo { } class TheClass { fn empty_method (Some(my self)) -> Foo { let foo = new Foo () ; foo ; } }, universe: universe(0), in_scope_vars: [], local_variables: [self : my TheClass, foo : Foo], existentials: [], assumptions: {} }, flow: Flow { moved_places: {self} } }` failed at the following rule(s):
                                 the rule "apply-perms" failed at step #0 (src/file.rs:LL:CC) because
                                   judgment had no applicable rules: `sub { a: shared (foo), b: my, env: Env { program: class Foo { } class TheClass { fn empty_method (Some(my self)) -> Foo { let foo = new Foo () ; foo ; } }, universe: universe(0), in_scope_vars: [], local_variables: [self : my TheClass, foo : Foo], existentials: [], assumptions: {} }, flow: Flow { moved_places: {self} } }`
                                 the rule "collapse a or b" failed at step #3 (src/file.rs:LL:CC) because
                                   cyclic proof attempt: `sub { a: shared (foo) Foo, b: Foo, env: Env { program: class Foo { } class TheClass { fn empty_method (Some(my self)) -> Foo { let foo = new Foo () ; foo ; } }, universe: universe(0), in_scope_vars: [], local_variables: [self : my TheClass, foo : Foo], existentials: [], assumptions: {} }, flow: Flow { moved_places: {self} } }`
                         the rule "type_expr_as" failed at step #1 (src/file.rs:LL:CC) because
