                                                                               ...
                                                                               judgment `sub_terms { terms_a: Terms { unique: false, shared: true, leased: false, vars: {}, named_tys: {(Terms { unique: false, shared: true, leased: false, vars: {}, named_tys: {}, shared_places: {d3}, leased_places: {} }, Data)}, shared_places: {d3}, leased_places: {} }, terms_b: Terms { unique: false, shared: true, leased: false, vars: {}, named_tys: {(Terms { unique: false, shared: true, leased: false, vars: {}, named_tys: {}, shared_places: {@ fresh(0) . pair}, leased_places: {} }, Data)}, shared_places: {@ fresh(0) . pair}, leased_places: {} }, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, @ fresh(0): Choice, d1: Data, d2: Data, d3: Data, pair: Pair, r: shared {d3} Data}, assumptions: {}, fresh: 1 }, flow: Flow { moved_places: {d1, d2, pair, r} } }` failed at the following rule(s):
                                                                                 the rule "sub_teams" failed at step #2 (src/file.rs:LL:CC) because
                                                                                   condition evaluted to false: `all_places_covered_by_one_of(&terms_a.shared_places, &terms_b.shared_places)`
                                                                                     &terms_a.shared_places = {d3}
                                                                                     &terms_b.shared_places = {@ fresh(0) . pair}
                                                                     the rule "cons" failed at step #5 (src/file.rs:LL:CC) because
                                                           ...
                                                           judgment `"flat_map"` failed at the following rule(s):
                                                             failed at (src/file.rs:LL:CC) because
                                                               judgment `field_of_accessed_place_prefix_permits_access { place_prefix: choice, field: data : leased {choice . pair} Data ;, place: choice . pair, access: lease, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, choice: Choice}, assumptions: {}, fresh: 0 }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                                                                 the rule "not accessed place" failed at step #2 (src/file.rs:LL:CC) because
                                                                   judgment `ty_permits_access { ty: leased {choice . pair} Data, access: lease, place: choice . pair, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, choice: Choice}, assumptions: {}, fresh: 0 }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                                                                     the rule "ty" failed at step #0 (src/file.rs:LL:CC) because
                                                                       judgment `perm_permits_access { perm: leased {choice . pair}, access: lease, place: choice . pair, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, choice: Choice}, assumptions: {}, fresh: 0 }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                                                                         the rule "disjoint" failed at step #0 (src/file.rs:LL:CC) because
                                                                           condition evaluted to false: `place_disjoint_from_all_of(&accessed_place, &perm_places)`
                                                           ...
                                                           judgment `"flat_map"` failed at the following rule(s):
                                                             failed at (src/file.rs:LL:CC) because
                                                               judgment `field_of_accessed_place_prefix_permits_access { place_prefix: choice, field: data : shared {choice . pair} Data ;, place: choice . pair, access: give, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, choice: Choice}, assumptions: {}, fresh: 0 }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                                                                 the rule "not accessed place" failed at step #2 (src/file.rs:LL:CC) because
                                                                   judgment `ty_permits_access { ty: shared {choice . pair} Data, access: drop, place: choice . pair, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, choice: Choice}, assumptions: {}, fresh: 0 }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                                                                     the rule "ty" failed at step #0 (src/file.rs:LL:CC) because
                                                                       judgment `perm_permits_access { perm: shared {choice . pair}, access: drop, place: choice . pair, env: Env { program: "...", universe: universe(0), in_scope_vars: [], local_variables: {self: my TheClass, choice: Choice}, assumptions: {}, fresh: 0 }, flow: Flow { moved_places: {} } }` failed at the following rule(s):
                                                                         the rule "disjoint" failed at step #0 (src/file.rs:LL:CC) because
                                                                           condition evaluted to false: `place_disjoint_from_all_of(&accessed_place, &perm_places)`
                                                                             &accessed_place = choice . pair
                                                                             &perm_places = {choice . pair}"#]])
