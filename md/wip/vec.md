...
#### 4c: Interpreter ✅
* [x] **`Bool` type and operators** — added `Bool` to `TypeName` (shared class, always copyable, 1 word, `Word::Int` representation). Added `true`/`false` as `Expr` variants with KEYWORDS. Added `Expr::Sub`, `Expr::Ge/Le/Gt/Lt/Eq/Ne` for subtraction and comparisons. Changed `Expr::If` condition from `Int` to `Bool`. Updated 5 existing tests.
