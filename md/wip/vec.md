...
* [ ] **Remove `Flags::Dropped`** — replace all uses of `Flags::Dropped` with `Word::Uninitialized`. When uninitializing a boxed type, write `Word::Uninitialized` for both the flags word and the pointer word (instead of `Word::Flags(Flags::Dropped)` + `Word::Uninitialized`). Remove `Flags::Dropped` from the `Flags` enum. `expect_flags()` stays unchanged — it still returns `Flags` and errors on non-`Word::Flags` input. Callers that currently match on `Flags::Dropped` (early return or bail) must instead check for `Word::Uninitialized` at the flags offset *before* calling `expect_flags()`. The pattern becomes: read the raw word, if `Word::Uninitialized` → handle the "already dropped" case, otherwise call `expect_flags()` and match on the remaining variants (`Given`, `Shared`, `Borrowed`). The error message becomes "access of uninitialized value" uniformly. See `uninitialize()`, `with_projection_flags()`, `read_word()`, `and_drop_fields()`, and the place-access match arms in `src/interpreter/mod.rs` — all follow this same pattern.
* [ ] **Scrub entire array backing on refcount zero** — when an array's refcount reaches 0, uninitialize *all* words in the backing allocation (elements included), not just the refcount and capacity header. This is the interpreter equivalent of freeing the allocation — it disappears from heap snapshots. Currently only the header words are scrubbed, leaving stale element data visible (e.g., `[Uninitialized, Uninitialized, Int(10), Int(20)]`). Note: this does NOT run element destructors — that's the user's responsibility via `array_drop`. This just zeroes the memory. See the `new_refcount == 0` branch in `and_drop_fields` in `src/interpreter/mod.rs`.
* [ ] **`Perm is Pred` syntax** — flip `Predicate::Parameter` grammar from `#[grammar($v0($v1))]` to `#[grammar($v1 is $v0)]`. Only affects `where` clause predicates, not class predicates (`given class`, `shared class`) or variance predicates (`Predicate::Variance` keeps `$v0($v1)`). Add `is` to KEYWORDS. Update all test programs and `where` clauses.

**TDD notes:** All existing tests should pass after mechanical rename/rewrite. No new tests needed — this is a syntax-only change. Heap snapshots may change slightly if any contained `Dropped` in their display format; update with `UPDATE_EXPECT=1` after verifying.
